...
**班次判定规则**（全表统一按「人」判定，同一人在所有页签班次一致）：
每个客服的整体班次由其登录模式推断——上午（12:00 前）登录过 → 白班；
只在午后/晚间登录 → 中班（中班 14:00-18:00、20:00-23:00，会提前几分钟登录）；
夜班名单内的人永远夜班，非名单的人永远不会被判成夜班。分配明细与按客服汇总
均使用该人的整体班次；仅当某人只有分配记录、无任何登录记录时，才退回按
记录时间落段（≤18:00 白班 / >18:00 中班）。
