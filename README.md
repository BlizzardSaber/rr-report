...
- 📊 **报表样式与现有表格一致** —— 两个工作表（近24小时分配数据 / agent上下线数据）、
  Arial 10、时间列 `yyyy-mm-dd hh:mm:ss`、冻结首行、自动筛选、🟢 在线中 / 已下线状态
...
通常不需要手改，用 `./manage.sh` 即可。`report` 两项控制报表范围：
分配表取近 24 小时（与原表一致）、上下线表取近 14 天（与接口保留期一致）。
