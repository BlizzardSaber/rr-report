...
**班次判定规则**（按「人 + 日期」逐天判定，班次随排班变动，不参考其他天的
历史）：每天只看当天的登录与分配行为——上午（12:00 前）有活动 → 当天白班
（证据优先）；无上午活动但晚间（18:00 后）有活动 → 当天中班（中班
14:00-18:00、20:00-23:00，必然参与晚间时段）；只有下午活动时，踩中班上班点
（14:00 前后 10 分钟内）开始的判中班，更晚才出现的判白班（临时支援场景）。
夜班名单内的人永远夜班，非名单的人永远不会被判成夜班。
报表每次都从全量数据重新生成，当天尚未结束时中班人员的晚间证据还未产生，
先按上班点判定，晚间数据到来后自动修正。
